...
export function generateLogs(type, { name } = {}, { name: playerName2, hp } = {}, valueAttack, $chat) {
...
function getTextLog(type, playerName1, playerName2) {
    switch (type) {
        ...
        case 'hit':
            return LOGS[type][getRandom(LOGS[type].length - 1) - 1].replace('[playerKick]', playerName1).replace('[playerDefence]', playerName2).replace('[time]', getTime());
        case 'defence':
            return LOGS[type][getRandom(LOGS[type].length - 1) - 1].replace('[playerKick]', playerName1).replace('[playerDefence]', playerName2).replace('[time]', getTime());
        case 'end':
            return LOGS[type][getRandom(LOGS[type].length - 1) - 1].replace('[playerWins]', playerName1).replace('[playerLose]', playerName2).replace('[time]', getTime());
        case 'draw':
            return LOGS[type];
    }
}
